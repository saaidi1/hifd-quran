...
@section('title', isset($groupe) ? __('Edit halaqa') : __('New halaqa'))
@section('page-title', isset($groupe) ? __('Edit halaqa') : __('New halaqa'))

@section('content')
    @php $g = $groupe ?? null; @endphp

    <form method="POST" action="{{ $g ? route('groupes.update', $g) : route('groupes.store') }}">
        @csrf
        ...
        <div class="app-card">
            <div class="card-body">
                <div class="row g-3">
                    ...
                    @unless (auth()->user()->estProfesseur())
                        <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="capacite" min="1" value="{{ old('capacite', $g?->capacite ?? 20) }}" class="form-control @error('capacite') is-invalid @enderror" required>
                        @error('capacite')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">{{ __('School year') }}</label>
                        <input type="text" name="annee_scolaire" value="{{ old('annee_scolaire', $g?->annee_scolaire ?? date('Y') . '/' . (date('Y') + 1)) }}" class="form-control">
                    </div>
                    <div class="col-md-4 d-flex align-items-end">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="actif" value="1" id="actif" @checked(old('actif', $g?->actif ?? true))>
